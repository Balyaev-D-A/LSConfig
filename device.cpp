...
bool Device::connectToDevice(QString port, uchar address)
{
    ...
    m_address = address;
    quint16 buffer;
    if (!readHoldingRegisters(CONFIGBASE + 0x280, 1, (char *)&buffer)) {
        m_pPort->close();
        return false;
    }
    if (buffer != address) {
...
bool Device::readHoldingRegisters(quint16 start, quint16 count, char *result)
{
    ...
    if (m_pPort->waitForReadyRead(m_settings->getConnSettings().requestTimeout)) {
        response = m_pPort->readAll();
        while(m_pPort->waitForReadyRead(m_settings->getConnSettings().packetTimeout))
        {
            response += m_pPort->readAll();
        }
    }
...
bool Device::getChannelConfig(uint channel, TChannelConfig *config)
{
    return readHoldingRegisters(CONFIGBASE + (channel * sizeof(TChannelConfig)), sizeof(TChannelConfig)/2, (char*) config);
}
...
bool Device::putChannelConfig(uint channel, TChannelConfig *config)
{
    return writeHoldingRegisters(CONFIGBASE + (channel * sizeof(TChannelConfig)), sizeof(TChannelConfig)/2, (char*) config);
}
